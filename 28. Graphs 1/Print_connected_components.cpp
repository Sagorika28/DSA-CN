...
void print_dfs(int **edge_mat, int v, int start_vertex, bool *vis, vector<int> &ans)
{
    ...
    for (int i = 0; i < v; i++)
    {
        ...
        {
            if (!vis[i])
                print_dfs(edge_mat, v, i, vis, ans);
        }
    }
}

vector<vector<int>> DFS(int **edge_mat, int v)
{
    // visited edges array
    bool *vis = new bool[v];
    for (int i = 0; i < v; i++)
...
int main(int argc, char const *argv[])
{
    ...
    // call dfs
    vector<vector<int>> result = DFS(edge_mat, v);
