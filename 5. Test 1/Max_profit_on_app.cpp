...
long long max_profit(long long *arr, long long n)
{
    long long profit = arr[0], curr = 0;
    sort(arr, arr + n); // sorting budgets in the increasing order
    for (int i = 0; i < n; i++)
    {
        // cost = budget*no.of people with budget = or higher
        curr = arr[i] * (n - i);
